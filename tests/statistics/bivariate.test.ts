...
describe('Bivariate and Matrix Statistical Functions', () => {
    describe('Contingency Table and Matrix Operations', () => {
        ...
        it('should extract a specific column correctly', () => {
            expect(getColumn(contingencyTable, 0)).toEqual(new Float64Array([10, 30]));
        ...
        it('should calculate combination table (marginal totals) correctly', () => {
            ...
            expect(comb).toEqual([
                ...
                new Float64Array([30, 40, 70]),
                new Float64Array([40, 60, 100])
            ]);
        });
    });
    ...
    describe('Chi-Square Calculations with Larger and Fractional Data', () => {
        it('Table 5 (4x3 matrix with fractions): should calculate chi-square correctly', () => {
            const fractionalTable1 = [
                new Float64Array([2.5, 5.5, 12.0]),
                new Float64Array([4.5, 6.5, 9.0]),
                new Float64Array([6.5, 8.5, 5.0]),
                new Float64Array([6.5, 9.5, 24.0])
            ];
        ...
        it('Table 6 (3x4 matrix with fractions): should calculate near-independent fractional table', () => {
            const fractionalTable2 = [
                new Float64Array([2.5, 6.5, 9.5, 11.5]),
                new Float64Array([5.5, 9.5, 14.5, 20.5]),
                new Float64Array([2.0, 4.0, 6.0, 8.0])
            ];
        ...
        it('Table 7 (5x5 matrix with fractions): should calculate chi-square for large uniform distribution with small perturbations', () => {
            const fractionalTable3 = [
                new Float64Array([20.5, 19.5, 21.0, 19.0, 20.0]),
                new Float64Array([18.5, 21.5, 19.5, 20.5, 20.0]),
                new Float64Array([22.0, 18.0, 20.5, 19.5, 20.0]),
                new Float64Array([19.0, 21.0, 19.0, 21.0, 20.0]),
                new Float64Array([20.0, 20.0, 20.0, 20.0, 20.0])
            ];
    ...
    describe('Independence Test and Association (Cramér V)', () => {
        ...
        it('Table 3 (3x2 non-square): should calculate exact Cramér V for non-square matrix', () => {
            const table = [
                new Float64Array([20, 6]),
                new Float64Array([25, 12]),
                new Float64Array([38, 40])
            ];
        ...
        it('Table 4 (3x3 square): should calculate exact Cramér V for 3x3 matrix', () => {
            const table = [
                new Float64Array([50, 10, 20]),
                new Float64Array([15, 45, 30]),
                new Float64Array([25, 20, 55])
            ];
    ...
    describe('ANOVA SSD and Eta Squared', () => {
        ...
        it('Table 3 (3 groups of unequal sizes): should handle groups with varying sample sizes', () => {
            const unequalGroups = [
                ...
                new Float64Array([20, 22]),
                new Float64Array([30, 32, 34, 36])
            ];
        ...
        it('Table 4 (4 groups of 4): should calculate exact SSD components and effect size for larger datasets', () => {
            const largerGroups = [
                new Float64Array([5, 7, 8, 10]),
                new Float64Array([12, 14, 15, 19]),
                new Float64Array([20, 21, 23, 24]),
                new Float64Array([30, 32, 35, 39])
            ];
    ...
    describe('Covariance and Pearson Correlation', () => {
        ...
        it('should calculate population and sample covariance', () => {
            ...
            expect(() => covariance(new Float64Array([1]), new Float64Array([2]), true)).toThrow('Sample covariance requires at least 2 data points.');
            expect(() => covariance(new Float64Array([1, 2]), new Float64Array([1]), false)).toThrow('The number of elements must match in the two arrays!');
        });

        it('should calculate Pearson correlation coefficient', () => {
            expect(correlation(x, y)).toBe(1);
            expect(correlation(new Float64Array([5, 5, 5]), new Float64Array([1, 2, 3]))).toBe(0);
            expect(correlation(new Float64Array([5, 7, 9, 11, 12, 23]), new Float64Array([1, 4, 6, 7, 9, 10]))).toBeCloseTo(0.8465, 4);
        });

        it('should correctly compute negative linear correlation', () => {
            const xVal = new Float64Array([1, 2, 3, 4, 5]);
            const yVal = new Float64Array([5, 4, 2, 1, 0]);

            const r = correlation(xVal, yVal, true, 4);
            expect(r).toBeCloseTo(-0.9912, 4);
        });

        it('should compute accurate correlation for real decimal data', () => {
            const xVal = new Float64Array([10, 20, 30, 40, 50]);
            const yVal = new Float64Array([12, 24, 28, 42, 58]);
        ...
        it('should be numerically stable with large numbers and offsets', () => {
            const xVal = new Float64Array([1e9 + 1, 1e9 + 2, 1e9 + 3, 1e9 + 4, 1e9 + 5]);
            const yVal = new Float64Array([100.5, 200.5, 300.5, 400.5, 500.5]);

            const r = correlation(xVal, yVal);
            expect(r).toBeCloseTo(1.0, 4);
        });

        it('should handle zero variance (constant array) safely without crashing', () => {
            const xVal = new Float64Array([5, 5, 5, 5, 5]);
            const yVal = new Float64Array([1, 2, 3, 4, 5]);
    ...
    describe('Ranks and Spearman Correlation', () => {
        describe('getRanks', () => {
            it('should compute fractional ranks correctly for ties and distinct values', () => {
                const ranks = getRanks(new Float64Array([10, 20, 20, 30]));
            ...
            it('should throw error when array length is less than 2', () => {
                expect(() => getRanks(new Float64Array([1]))).toThrow('Values array must contain at least 2 numbers!');
            });
        });
        ...
        describe('rankCorrelation (Spearman)', () => {
            it('should calculate exact Spearman rank correlation coefficient', () => {
                const x = new Float64Array([1, 2, 3, 4, 5]);
                const y = new Float64Array([1, 2, 4, 3, 5]);
            ...
            it('should return 1 for perfectly monotonic increasing data', () => {
                ...
                const y = new Float64Array([5, 15, 25, 35]);
            ...
            it('should return -1 for perfectly monotonic decreasing data', () => {
                ...
                const y = new Float64Array([40, 30, 20, 10]);
                expect(rankCorrelation(x, y)).toBe(-1);
            });

            it('should correctly handle tied ranks', () => {
                const x = new Float64Array([10, 20, 20, 30, 40]);
                const y = new Float64Array([1, 2, 3, 3, 5]);
            ...
            it('should calculate Spearman correlation for small sample with single tie', () => {
                const x = new Float64Array([5, 10, 10, 15, 20]);
                const y = new Float64Array([2, 4, 6, 6, 10]);
            ...
            it('should calculate Spearman correlation with multiple and triple ties', () => {
                const x = new Float64Array([10, 10, 10, 20, 30, 40]);
                const y = new Float64Array([5, 15, 25, 25, 25, 50]);
            ...
            it('should calculate Spearman correlation for negative association with ties', () => {
                const x = new Float64Array([1, 2, 3, 3, 5, 6]);
                const y = new Float64Array([10, 8, 8, 4, 2, 1]);
            ...
            it('should calculate Spearman correlation when values repeat in sequence', () => {
                const x = new Float64Array([100, 100, 200, 200, 300, 300]);
                const y = new Float64Array([10, 30, 20, 40, 30, 50]);
            ...
            it('should throw error if input arrays have different lengths', () => {
                expect(() => rankCorrelation(new Float64Array([1, 2]), new Float64Array([1]))).toThrow();
            });
        });
    });
});