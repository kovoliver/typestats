...
describe('Regression Class', () => {
    ...
    describe('Advanced Regression & RSD Validation with Precise Expectations', () => {
        it('should calculate linear regression correctly for negative and decimal values (10+ elements)', () => {
            const x = [-5.25, -4.10, -3.15, -2.05, -1.10, 0.20, 1.15, 2.30, 3.40, 4.55];
            const y = [-12.85, -10.20, -8.15, -5.90, -3.40, -0.15, 2.45, 5.10, 7.80, 10.35];
            const reg = new Regression(x, y);
            const result = reg.linear();
        ...
        it('should calculate exponential regression and RSD precisely across 100s, 1000s, and 10k scales', () => {
            const x100 = [100.25, 105.50, 110.75, 115.20, 120.40, 125.80, 130.15, 135.60, 140.90, 145.30];
            const y100 = [205.54, 223.12, 242.21, 262.93, 285.42, 309.83, 336.32, 365.07, 396.28, 430.15];
            ...
            const x1000 = [1100.5, 1220.4, 1340.2, 1460.8, 1580.1, 1700.5, 1820.3, 1940.9, 2060.2, 2180.7];
            const y1000 = [150.25, 170.85, 194.25, 220.85, 251.15, 285.65, 324.85, 369.45, 420.25, 478.05];
            ...
            const x10k = [10050.2, 11020.5, 12010.8, 13040.1, 14025.4, 15060.7, 16010.3, 17050.6, 18020.9, 19040.2];
            const y10k = [450.12, 490.25, 533.90, 581.40, 633.15, 689.50, 750.90, 817.85, 890.80, 970.30];
        ...
        it('should calculate power regression and RSD precisely across 100s, 1000s, and 10k scales', () => {
            const x100 = [110.5, 125.2, 140.8, 155.1, 170.4, 185.7, 200.2, 215.9, 230.1, 245.6];
            const y100 = [12.45, 14.10, 15.80, 17.35, 18.95, 20.50, 22.05, 23.60, 25.10, 26.65];
            const reg100 = new Regression(x100, y100);
            ...
            const x1000 = [1150.2, 1300.5, 1450.8, 1600.1, 1750.4, 1900.7, 2050.3, 2200.6, 2350.9, 2500.2];
            const y1000 = [45.20, 51.10, 56.80, 62.30, 67.75, 73.10, 78.40, 83.65, 88.85, 94.05];
            const reg1000 = new Regression(x1000, y1000);
            ...
            const x10k = [10500.2, 12050.5, 13500.8, 15050.1, 16500.4, 18050.7, 19500.3, 21050.6, 22500.9, 24050.2];
            const y10k = [120.50, 137.80, 154.20, 171.10, 187.35, 204.05, 220.15, 236.80, 252.85, 269.45];
            const reg10k = new Regression(x10k, y10k);
